...
public class BypassCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(Utils.Color(plugin.getConfig().getString("only-player")));
            return false;
        }
        if (args.length != 1) {
            player.sendMessage(Utils.Color(plugin.getConfig().getString("command-usage").replace("%label%", label)));
            return false;
        }
        if (!player.hasPermission("arenaprotector.bypass")) {
            player.sendMessage(Utils.Color(plugin.getConfig().getString("no-perm")));
            return false;
        }
        if (ArenaProtector.playersWhoBypassEnabled.contains(player.getUniqueId())) {
            ...
            player.sendMessage(Utils.Color(plugin.getConfig().getString("admin-bypass-off").replace("%player%", player.getName())));
            return false;
        }
        ArenaProtector.playersWhoBypassEnabled.add(player.getUniqueId());
        player.sendMessage(Utils.Color(plugin.getConfig().getString("admin-bypass-on").replace("%player%", player.getName())));
        return false;
    }
}
